...
def generate_real_samples_with_labels_Rayleigh(h_dataset, number=100):
    ...
    """
    QAM = np.asarray([-3 - 3j, -3 - 1j, -3 + 1j, -3 + 3j, -1 - 3j, -1 - 1j, -1 + 1j, -1 + 3j,
                               1 - 3j, 1 - 1j, 1 + 1j, 1 + 3j, 3 - 3j, 3 - 1j, 3 + 1j, 3 + 3j   ], dtype=np.complex64)

    h = np.random.choice(h_dataset, size=number)
    x = np.random.choice(QAM, size=number)

    h = h / np.sqrt(np.mean(np.abs(h_dataset)**2))

    
    noise_variance = 0.035
    n = (np.random.normal(scale=np.sqrt(noise_variance), size=number) + 
             1j * np.random.normal(scale=np.sqrt(noise_variance), size=number))
    
    y = h * x + n
    receive = np.column_stack((np.real(y), np.imag(y)))

 
    sit = np.column_stack((np.real(x),np.imag(x),np.real(h),np.imag(h) ))
